...
interface Props {
  ...
  customBorder?: string
  filterStyle?: 'grayscale' | 'grayscale-emoji' | 'sepia' | 'cyan-tone' | 'none'
...
const getColorClasses = () => {
  if (props.colorScheme === 'custom' && props.customGradient && props.customBorder) {
    return {
  ...
  const schemes: Record<'cyan' | 'emerald' | 'purple', { gradient: string; border: string }> = {
    emerald: {
  ...
  return props.colorScheme === 'custom' ? schemes.cyan : schemes[props.colorScheme]
}
...
<template>
  ...
  <div v-if="variant === 'timeline'" class="flex items-start gap-4 flex-1">
    <div
      ...
      :class="[`bg-gradient-to-br ${colors.gradient}`, colors.border, sizes.box]"
      :style="applyToBox ? getEmojiFilterStyle() : ''"
    >
      <span :class="sizes.emoji" :style="applyToEmoji ? getEmojiFilterStyle() : ''">
        {{ emoji }}
      </span>
    </div>
    <div class="flex-1 pt-2">
      <h5 class="font-bold text-white mb-1" :class="sizes.label">{{ label }}</h5>
