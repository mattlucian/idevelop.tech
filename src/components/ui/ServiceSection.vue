...
<template>
  ...
  <div v-if="variant === 'hero'" class="max-w-5xl mx-auto px-6 py-8">
    <div :class="$slots.default ? 'flex flex-col md:flex-row gap-8 md:gap-12' : ''">
      ...
      <div v-if="$slots.default" class="w-full md:w-[340px] lg:w-[400px] flex-shrink-0">
  ...
  <div v-else-if="variant === 'benefits'" class="max-w-5xl mx-auto px-6 py-16">
    <!-- Section Title -->
    <h2 v-if="title" class="text-3xl md:text-4xl font-bold text-white mb-12 text-center">
      {{ title }}
    </h2>
    ...
    >
      ...
      <p class="text-slate-400 max-w-2xl mx-auto">{{ expertiseBadge.description }}</p>
    </div>
  </div>

  <!-- Portfolio Variant -->
  <div v-else-if="variant === 'portfolio'" class="bg-slate-900/30 border-y border-slate-800/30">
    <div class="max-w-5xl mx-auto px-6 py-16">
      <h2 :class="['text-sm font-semibold uppercase tracking-wider mb-8', colorClasses.text]">
        {{ title }}
      </h2>
